...
- **Consulta ao PubMed (compêndios de medicina):** antes de fechar qualquer compêndio médico novo ou reescrita ≥30%, usar o conector PubMed (`mcp__9fb1d51e-0387-4936-8e13-b066ab070600__search_articles` ou `get_full_text_article`) para: (a) verificar se há revisões sistemáticas ou meta-análises recentes (últimos 3–5 anos) sobre o mecanismo central; (b) identificar controvérsias ativas na literatura que devem aparecer em `.kbox` "Controvérsia"; (c) enriquecer a seção "Perguntas em aberto" com questões genuinamente não resolvidas. Registrar DOIs relevantes nas Referências ABNT. Não buscar para cada parágrafo — focar nos mecanismos centrais e nas fronteiras do conhecimento.
...
## Encerramento de sessão — autocrítica obrigatória

Ao final de qualquer sessão de trabalho substantivo (novo compêndio, migração, reescrita, atualização de instruções), chamar a skill `autocritica-sessao` antes de fechar. A skill avalia padrões da sessão, identifica regras implícitas que deveriam ser formalizadas, detecta inconsistências nas instruções e propõe melhorias. Não é opcional — é parte do ciclo de melhoria contínua do projeto.

Sinal de encerramento: usuário diz "terminamos", "pode fechar", "acabou", "sessão encerrada" ou pergunta sobre melhorias ao final.
...
O critério de parada é uma releitura limpa (sem novas correç