...
Duas convenções de `_archive/` coexistem no projeto, ambas válidas — não é erro, é uma divergência documentada aqui para não gerar confusão em sessão futura: `compêndios/`, `index.html` e os arquivos centrais de `_docs/` usam um `_archive/` único na raiz do projeto; os compêndios de mecanismo (hoje em `compêndios/medicina/mecanismos/`, migrado de `medicina/` em 2026-07-10) usam um `_archive/` aninhado por subpasta (ex.: `compêndios/medicina/mecanismos/imunologia/_archive/`, `compêndios/medicina/mecanismos/fisiopatologia/_archive/`, `compêndios/medicina/mecanismos/farmacologia/_archive/`). Ao arquivar um compêndio de mecanismo, seguir o padrão aninhado já em uso na subpasta; ao arquivar um compêndio de área ou arquivo central de `_docs/`, usar o `_archive/` da raiz. Não migrar arquivos de uma convenção para outra sem motivo — a coexistência é aceita, não uma dívida a resolver.
...
- Algum `SKILL.md` editado → Claude já empacota e apresenta o `.skill` atualizado via `present_files` (nomeando cada skill) antes de encerrar; a única pendência do usuário é clicar "Save skill" em cada card.
