# Mapa de Materiais — Mecanismo (compêndios/medicina/mecanismos/)

Registro de conceitos centrais cobertos por compêndio de MECANISMO (subskill-folha `mecanismo`, pasta `compêndios/medicina/mecanismos/`).
Consultar antes de escrever explicação inline — linkar via `a.cross-link` em vez de reexplicar.

Escopo deste arquivo: apenas `compêndios/medicina/mecanismos/` (mecanismo). Compêndios de ÁREA (skill `compendio`, pasta `compêndios/`)
têm mapa próprio em `compêndios/compendio_mapa.txt`. Compêndios de SISTEMA (domínio Sistemas, pasta `sistemas/`) têm mapa próprio
...
ver `_docs/DEBT.md` para o histórico da correção. **Caminho raiz atualizado em 2026-07-10** (migração de `medicina/` para `compêndios/medicina/mecanismos/`, desmembramento de Base de estudos em Compêndios/Sistemas/Provas) — só o prefixo de pasta mudou, os arquivos abaixo são os mesmos.
...
## Microbiologia (`compêndios/medicina/mecanismos/microbiologia/`) — 2 compêndios ativos
...
## Semiologia (`compêndios/medicina/mecanismos/semiologia/`) — 1 compêndio ativo

- `sindromes-bronco-pleuro-pulmonares.html` — Síndromes Bronco-Pleuro-Pulmonares: mecanismo físico central (impedância acústica × densidade/ressonância) por trás do FTV, percussão e ausculta; síndromes brônquicas (asma, DPOC, bronquiectasias), parenquimatosas (condensação, atelectasia, enfisema, congestão passiva, cavitação), pleurais (derrame, pneumotórax) e diafragmáticas/mediastínicas (paralisia diafragmática, SVCS)
...
- Síndrome de congestão pulmonar passiva (semiologia) → `fisiopatologia/doencas-circulatorias.html` (mecanismo geral de congestão/edema)
- Síndromes brônquicas/pleurais infecciosas (semiologia) → `../infectologia-fundamentos.html` (síndromes infecciosas) e `imunologia/imunidade-inata.html` (resposta inflamatória)
